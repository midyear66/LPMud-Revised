...
- **Underground Rail** — Fast-travel mine cart system connecting five stations: Village, Eastroad Inn, Plains Crossing, Orc Valley, and Solace. The cart runs a continuous loop on a ~75-second circuit, pausing 10 seconds at each stop. Players `board` the cart and `exit` at their destination. Stations are accessed via `down` from surface rooms. Train object: `obj/train.c`, station rooms: `room/station.c`, `room/station_east.c`, `room/station_plains.c`, `room/station_orc.c`, `room/dragonlance/solace/station.c`, sign: `obj/sign.c`.
...
- **Dragonlance Area** — 48-room themed area inspired by the Dragonlance Chronicles (Weis & Hickman), entered from the western plains (`room/plane10`). Four sub-areas spanning all level ranges: **Solace** (tree-top village, levels 1-5) with the Inn of the Last Home and the Blue Crystal Staff quest; **Xak Tsaroth** (ruined city, levels 5-10) with draconians and the black dragon Khisanth guarding the Disks of Mishakal; **Pax Tharkas** (mountain fortress, levels 10-15) occupied by the Dragonarmy under Highlord Verminaard; and the **Temple of the Dark Queen** (level 16+ portal) featuring the Avatar of Takhisis as the final boss. Four quests with iconic NPCs (Goldmoon, Riverwind, Fizban/Paladine, Elistan, Laurana), each awarding XP and unique equipment. Rooms organized in `room/dragonlance/{roads,solace,xak_tsaroth,pax_tharkas,temple}/`. Controllers: `obj/dl_staff_quest.c`, `obj/dl_disks_quest.c`, `obj/dl_pax_quest.c`, `obj/dl_takhisis_quest.c`.
- **Weapon/armor shop system** — Reusable shop base (`obj/weapon_shop.c`) that any room can inherit to become a weapon and armor shop with buy/sell/list/value commands. Shops stock a fixed set of always-available items (restocked on reset) plus 5-10 random items from a configurable pool. Items are hidden from the room inventory listing and protected from NPC pickup. Both the village shop (`room/shop.c`) and the Solace shop (`room/dragonlance/solace/shop.c`) use the system, each with their own shopkeeper NPC and level-appropriate stock. Sold items go to a shared consignment store (`room/store.c`) available at both shops.
- **Bug fixes** — Runtime error logging in `obj/master.c`, relaxed wizard level check in `obj/leo.c`, `query_ip_name`/`query_ip_number`/`query_idle` crash guard for non-interactive objects in `obj/simul_efun.c`, wyrm gem drop fix in `room/south/lair.c` (sapphire could never drop due to off-by-one). Roommaker fixes: array overflow in max exits check, broken `long()` display string, generated code type mismatch in `query_room_maker()`, off-by-one in direction editor bounds, overly broad filename validation. Player teleport fix: error message now shows original filename instead of "0". Elevator typo fix. Well and door trap logic fixes: `&&`→`||` in open/close null-check, and `open` action was incorrectly bound to the `close` verb. Uptime display fix: church clock and wizard workroom orb now read boot time from the master object so `update` no longer resets the uptime counter. The master itself now sources boot time from `driver_info(DI_BOOT_TIME)` instead of a static variable, so a master recompile no longer zeros the uptime either. Singular/plural grammar fix for soul count in workroom orb. Docker networking fix: MUD container switched from bridge networking (`ports:`) to `network_mode: host` so `query_ip_number()` returns real client IPs instead of the Docker gateway address — fixes "last login" display and IP logging.
...
- **`world-map.png`** — Visual map of the game world (~292 rooms across 16+ regions)
