...
The world map is auto-generated by `scripts/generate-map.py`, which parses all room `.c` files in the mudlib to discover rooms and connections, then renders a PNG using [Pillow](https://pillow.readthedocs.io/) and [NetworkX](https://networkx.org/). Rooms are laid out on per-region grids with compass-faithful BFS placement and orthogonal connection routing.
