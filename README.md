...
- **Wizard Familiar** — Remote eyes-and-ears NPC controlled via Peace's wizard staff. Conjure a familiar with a custom description, send it to any room, see what it sees, hear what others say, and speak through it. Commands: `familiar create|look|move|say|emote|send|recall|where|mute|destroy` (or `fm` shorthand). Staff also includes `staff help` and teleport commands (`church`, `workshop`). Files: `players/peace/familiar.c`, `players/peace/staff.c`.
