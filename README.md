...
The `docs/` directory contains world map documentation generated from the mudlib's room files:

- **`world-map.png`** — Visual map of the game world
- **`world-map.txt`** — ASCII representation of the map
- **`room-connectivity.txt`** — Complete room connection graph

These were generated using `scripts/generate-map.py`, which parses room files for exit definitions and builds a connectivity graph.
...
Parses all room files in `mudlib/` for exit definitions and generates the map files in `docs/`.
