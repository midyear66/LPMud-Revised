/*
 * Orc Valley Station - Underground rail stop beneath the orc valley.
 */
...
void reset(int arg) {
    if (arg) return;
    set_light(1);
    short_desc = "Orc Valley Station";
    dest_dir = ({ "room/orc_vall", "up" });
    items = ({
...
void long(string str) {
    ...
    write("You are in an underground station carved from solid rock. Steel rails\n" +
	  ...
	  "A wooden sign shows the rail schedule. Stone stairs lead up to the orc valley.\n");
